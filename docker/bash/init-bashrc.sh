...
WEBSITE80_PATH="/data/www/default"
LOGS_PATH="/data/logs"
CONF_PATH="/data/env"
SSH_PATH="/root/.ssh"
...
\cp -fr ${CONF_PATH}/nginx/conf/conf.d/*.conf /etc/nginx/sites-enabled/
...
if [ -f "${CONF_PATH}/ssh-key/config" ]; then
  \cp -fr ${CONF_PATH}/ssh-key/config /root/.ssh/
fi

if [ -f "${CONF_PATH}/ssh-key/known_hosts" ]; then
  \cp -fr ${CONF_PATH}/ssh-key/known_hosts /root/.ssh/
fi
